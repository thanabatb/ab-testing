import { BriefcaseBusiness, ChevronLeft, Home, MessageCircleMore, ScanLine, Signal, UsersRound, Wifi } from "lucide-react";
import Link from "next/link";
import type { ComponentType } from "react";

import { cn } from "@/lib/utils";

import type { Contact } from "../data/phone-book-version-a.data";
...
type PhoneBookVersionAFavoritesScreenProps = {
  contacts: Contact[];
};
...
function FavoriteCard({ contact }: { contact: Contact }) {
  return (
...
export function PhoneBookVersionAFavoritesScreen({ contacts }: PhoneBookVersionAFavoritesScreenProps) {
  return (
    <main className="min-h-screen bg-[#e8eaee] sm:py-6">
      <section className="relative mx-auto min-h-screen w-full max-w-[440px] overflow-hidden bg-[#f1f2f4] sm:min-h-[956px] sm:rounded-[28px] sm:shadow-2xl">
        <div className="absolute inset-x-0 top-0 h-[220px] bg-[linear-gradient(180deg,#2e8bd1_0%,#64ddf9_50.96%,rgba(100,221,249,0)_100%)]" />
        ...
        <div className="relative z-10 px-4 pt-5 text-[#454545]">
          ...
          <h1 className="mt-4 text-[22px] font-bold leading-[1.3] text-[#111111]">รายการโปรด</h1>
          <p className="mt-1 text-[16px] font-light leading-[1.3] text-black">{contacts.length} รายการ</p>
        </div>

        <section className="relative z-10 px-4 pb-[112px] pt-4">
          <div className="flex flex-col gap-4">
            {contacts.map((contact) => (
              <FavoriteCard key={contact.id} contact={contact} />
            ))}
          </div>
        </section>
