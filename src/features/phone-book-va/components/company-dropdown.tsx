...
export function CompanyDropdown({ avatarSrc, companies, defaultCompany }: CompanyDropdownProps) {
  ...
  return (
    <div ref={containerRef} className="relative flex items-center gap-6">
      <div className="relative size-[60px] overflow-hidden rounded-[16px] bg-[#f3a7ad]">
        <img src={avatarSrc} alt="profile" className="size-full object-cover" width={60} height={60} />
      </div>
      ...
      <div className="relative">
        ...
        >
          <p className="text-[22px] font-bold leading-[1.3] text-black">{selectedCompany}</p>
          ...
          <ul className="absolute left-0 z-30 mt-2 w-[220px] rounded-[16px] border border-[#dbe2eb] bg-white p-2 shadow-[0_8px_24px_rgba(49,144,247,0.18)]">
            {companies.map((company) => (
              <li key={company}>
                <button
                  type="button"
                  className="w-full rounded-[10px] px-3 py-2 text-left text-[14px] text-[#111111] hover:bg-[#f4f8fd]"
                  onClick={() => {
